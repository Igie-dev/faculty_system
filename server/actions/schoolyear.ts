...
import {
 schoolyear,createSchoolyearSchema
...
export const createSchoolYear = async(prevState:TFormState,data:FormData):Promise<TFormState> => {
    ...
    const parsed = createSchoolyearSchema.safeParse(formData);
    if (!parsed.success) {
    ...
    const save = await db.insert(schoolyear).values({
...
export const getAllSchoolYearQuery = async():Promise<{data?:TSchoolYearData[], error?:string}> => {
    try{
...
export const getSchoolYearQuery = async(schoolyearId:string):Promise<{data?:TSchoolYearData,error?:string}> => {
    try{
...
export const deleteSchoolYearById = async(id:number): Promise<{ message?: string; error?: string }> => {
    try{
        const user = await getCurrentUser();
        if (user?.role !== ERole.IS_ADMIN) {
          return {
