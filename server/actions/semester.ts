...
export const createSemester = async(prevState:TFormState,data:FormData):Promise<TFormState> => {
    try{
    ...
    const parsed = createSemesterSchema.safeParse(formData);
    if (!parsed.success) {
    ...
    const ordinal = Number(formData.semester) >= 4 ? "th" : ordinalIndicators[Number(formData.semester)-1];
    ...
    const save = await db.insert(semester).values({
...
export const getAllSemesterQuery = async():Promise<{data?:TSemesterData[], error?:string}> => {
    try{
...
export const getSemesterQuery = async(semesterId:string):Promise<{data?:TSemesterData,error?:string}> => {
    try{
...
export const deleteSemesterById = async(id:number):Promise<{ message?: string; error?: string }>  => {
    try{
        const user = await getCurrentUser();
        if (user?.role !== ERole.IS_ADMIN) {
          return {
